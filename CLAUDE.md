...
├── constraints.txt             # Torch version pins (generated by rebuild script)
...
- **whisper-ptt.ahk** — AutoHotKey v2.0+ hotkeys: F8 (hold=record, release=transcribe+paste), F7 (start daemon), F9 (shutdown). Contains hardcoded absolute paths that must be updated if the project moves.
...
- **torch/torchvision/torchaudio** — Must stay as matching CUDA 11.8 trio (`+cu118` wheels).
...
- CUDA Toolkit v12.9 (but PyTorch wheels are cu118)
- Whisper model uses ~2 GB VRAM; diarization uses ~1-2 GB per subprocess
...
Required for pyannote model downloads. Read from `hf_token.txt` in project root or environment variables (`HF_TOKEN`, `HUGGINGFACE_TOKEN`, `HUGGINGFACEHUB_API_TOKEN`).